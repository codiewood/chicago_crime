...
```{r}
FullBinTest <- rbind(up,down,logistic,repSVM) %>%  mutate(Model = as.factor(Model), ACC = as.numeric(ACC), SENS = as.numeric(SENS), SPEC = as.numeric(SPEC), BALACC = as.numeric(BALACC))
FullBinTest %>% group_by(Model) %>% summarise(across(everything(), mean))
```
```{r}
ACCPlot <- ggplot(FullBinTest, aes(x = Model, y = ACC)) + geom_boxplot() + geom_hline(mapping = aes(yintercept = 0.7828), color = 'red')
