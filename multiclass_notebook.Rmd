...
BEEP below code to encode time as a category
```{r}
x <- lubridate::hour(dat$Date) + lubridate::minute(dat$Date)/60
tod <- rep("NIGHT",length(x))
for(i in 1:length(x)){
  if(5 < x[i] & x[i] < 12){
    tod[i] <- "MORNING"
    }
  else if(12 < x[i] & x[i] < 17){
    tod[i] <- "AFTERNOON"
    }
  else if(17 < x[i] & x[i] < 21){
    tod[i] <- "EVENING"
    }
}
tod <- as.factor(tod)
...
```{r}
ggplot(dat) +
  geom_bar(aes(y = `Time of Day`, colour=Arrest))
```
...
y <- dat$`Primary Type`
X_area <- dat %>% select(-c(`Primary Type`,`Location Description`,District, `Time of Day`))
X_district <- dat %>% select(-c(`Primary Type`,`Location Description`,`District Area`, `Time of Day`))
```
...
tod_mnlr_results_area <- mnlr_kfold_cv(X = X_area, y = y, k = 5, n_reps = 5, metrics = metrics)
tod_mnlr_results_district <- mnlr_kfold_cv(X = X_area, y = y, k = 5, n_reps = 5, metrics = metrics)
```
