...
dat$`Primary Type` <- dat$`Primary Type` %>%
  fct_infreq() %>%
  fct_rev()

thr <- 1000 # set threshold for othering

plot_factors(dat, "Primary Type", threshold = thr)
...
dat$`Location Description` <- dat$`Location Description` %>%
  fct_infreq() %>%
  fct_rev()

plot_factors(dat, "Location Description", thr)
```
...
plot_factors(dat, "Location Description")
```
...
plot_factors(dat, "District Area")
```
