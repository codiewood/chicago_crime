...
df <- read_csv("C:/Users/Rahil/OneDrive/Documents/ProjectSC1/data_raw.csv", show_col_types = FALSE) %>%
...
week_test <- week_test %>% 
  cbind("Predicted" = as.numeric(predict(model_week, week_test, type = "response")), "Data" = "Test")
...
crimes <- crimes %>%
  select(- c(Date,  'X Coordinate','Y Coordinate','District Area', 'Location Description'))
...
mnlr_results_district.df <- 
  mnlr_kfold_cv.df(X = X_district, y = y, k = 5, n_reps = 5, metrics = metrics)
```
...
```{r, eval=FALSE}
tod_results_noloc <- 
  mnlr_kfold_cv.df(X = X_tod_noloc, y = y_tod, k = 5, n_reps = 5, metrics = metrics)
```

```{r, eval = FALSE}
tod_results_area <- 
  mnlr_kfold_cv.df(X = X_tod_area, y = y_tod, k = 5, n_reps = 5, metrics = metrics)
```

```{r, eval=FALSE}
tod_results_district <- 
  mnlr_kfold_cv.df(X = X_tod_district, y = y_tod, k = 5, n_reps = 5, metrics = metrics)
```
