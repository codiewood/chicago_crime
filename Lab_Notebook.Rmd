...
df <- df %>%
  select(-c(ID, `Case Number`, `Updated On`, 
            `Block`, `Beat`, `Ward`, `X Coordinate`, `Y Coordinate`, `Latitude`, `Longitude`,
            `IUCR`, `FBI Code`, `Description`)) %>%
  drop_na()
```
...
plot_factors(df, "District")
```

In order to simplify analysis, we exclude the crimes from District 031, 16 and 021, and simply note that as a result our model will not be suitable for prediction in this district. As such we are left with 22 districts in our analysis.
...
df <- df %>%
  filter(!(District == "031"), !(District == "16"), !(District == "021")) %>%
...
We can also visualise the predicted values, true counts and residuals on a map for the first month of the test data:
...
true_plot <- plot_heat_map(plot_dat, "Count", trans = "reverse")

fit_plot <- plot_heat_map(plot_dat, "Fitted", trans = "reverse")

res_plot <- plot_heat_map(plot_dat, "Residuals")

gridExtra::grid.arrange(true_plot, fit_plot, res_plot)

```

We can see apart from a few areas the model performs well.
...
We can see this does not contribute meaningfully to the model so we keep the original spatial model
...
crimes <- crimes %>%
  ...
  mutate(day = as.factor(day), wkday = as.factor(wkday)) %>%
  droplevels() %>% 
  drop_na()
...
crimes <- crimes %>%
  select(- c(Date, Year, Location, `Community Area`, Location))
